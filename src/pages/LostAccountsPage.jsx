...
import {
    ...
    Users, RefreshCw, Award, Info, ChevronRight, ChevronLeft, Upload, Download,
    Calendar, TrendingUp, PieChart as PieChartIcon, Building, Target
} from 'lucide-react';
...
const COLORS = ['#E30613', '#FF6B6B', '#FFA726', '#66BB6A', '#42A5F5', '#AB47BC', '#26C6DA', '#FFCA28', '#8D6E63', '#78909C'];
...
const LostAccountsPage = ({ onBack, userRole }) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await fetch('http://localhost:8000/api/lost-accounts');
            if (!response.ok) {
    ...
    const handleDownloadTemplate = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/download-lost-accounts-template?year=${selectedYear}`);
            ...
            link.setAttribute('download', `lost_accounts_${selectedYear}_template.csv`);
    ...
    const handleUploadCSV = async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:8000/api/upload-lost-accounts', {
                method: 'POST',
                body: formData,
            });
    ...
    const handleDownloadVariousTemplate = async () => {
        try {
            const response = await fetch('http://localhost:8000/api/download-lost-various-template');
            if (!response.ok) throw new Error('Failed to download template');
            const result = await response.json();
    ...
    const handleUploadVariousCSV = async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:8000/api/upload-lost-various', {
                method: 'POST',
                body: formData,
            });
    ...
    const currentYearData = filteredRawData.filter(item => item.year === currentYear);
    const previousYearData = filteredRawData.filter(item => item.year === previousYear);
    ...
    currentYearData.forEach(item => {
        if (monthlyDataCurrentYear[item.month]) {
            ...
            monthlyDataCurrentYear[item.month].revenue += parseFloat(item.amount) || 0;
            
            if (item.division) {
                monthlyDataCurrentYear[item.month].accountsByDivision[item.division] = 
                    (monthlyDataCurrentYear[item.month].accountsByDivision[item.division] || 0) + 1;
            }
        }
    });
    ...
    const momComparison = MONTH_ORDER.map(month => {
        const currentYearMonthData = currentYearData.filter(item => item.month === month);
        const previousYearMonthData = previousYearData.filter(item => item.month === month);
        
        const currentAccounts = currentYearMonthData.length;
        const previousAccounts = previousYearMonthData.length;
        
        const currentRevenue = currentYearMonthData.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
        const previousRevenue = previousYearMonthData.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
        
        const accountGrowth = previousAccounts > 0 ? 
            (currentAccounts - previousAccounts) / previousAccounts : 
            (currentAccounts > 0 ? 1 : 0);
    ...
    currentYearData.forEach(item => {
        if (item.division) {
            ...
            divisionDataCurrentYear[divisionName].revenue += parseFloat(item.amount) || 0;
        }
    });

    const divisionArrayCurrentYear = Object.values(divisionDataCurrentYear).sort((a, b) => b.accounts - a.accounts);

    // Year-over-Year Comparison (based on filtered data)
    const yoyComparison = {
        currentYear: {
            accounts: currentYearData.length,
            revenue: currentYearData.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0)
        },
        lastYear: {
            accounts: previousYearData.length,
            revenue: previousYearData.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0)
        }
    };

    // Calculate growth percentages
    yoyComparison.accountsGrowth = yoyComparison.lastYear.accounts > 0 ?
        (yoyComparison.currentYear.accounts - yoyComparison.lastYear.accounts) / yoyComparison.lastYear.accounts : 
        (yoyComparison.currentYear.accounts > 0 ? 1 : 0);
    
    yoyComparison.revenueGrowth = yoyComparison.lastYear.revenue > 0 ?
        (yoyComparison.currentYear.revenue - yoyComparison.lastYear.revenue) / yoyComparison.lastYear.revenue :
        (yoyComparison.currentYear.revenue > 0 ? 1 : 0);
    ...
    return {
        ...
        totalLostRevenue: currentYearData.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0),
    ...
    const sortedReasons = [...(lostByReason || [])]
        .sort((a, b) => b.value - a.value);
                ...
                <header className="mb-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
                        <div>
                            ...
                            <p className="text-lg text-gray-600 mt-2">
                                Comprehensive insights into client churn and retention opportunities
                            </p>
                        </div>
                        ...
                        <div className="flex space-x-2 bg-gray-100 p-1 rounded-lg">
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                                    activeTab === 'overview' ? 'bg-white shadow-sm text-red-600' : 'text-gray-600 hover:text-gray-800'
                                }`}
                            >
                                Overview
                            </button>
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                                    activeTab === 'analytics' ? 'bg-white shadow-sm text-[#E30613]' : 'text-gray-600 hover:text-gray-800'
                                }`}
                            >
                                Advanced Analytics
                            </button>
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                                    activeTab === 'details' ? 'bg-white shadow-sm text-[#E30613]' : 'text-gray-600 hover:text-gray-800'
                                }`}
                            >
                                Details
                            </button>
                    ...
                    {isAdmin && (
                        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <h3 className="text-lg font-semibold text-gray-800 mb-3">Lost Accounts Data</h3>
                        ...
                        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                            <h3 className="text-lg font-semibold text-gray-800 mb-3">Various Divisions Data</h3>
                            <div className="flex gap-2">
                                <button 
                ...
                {activeTab === 'overview' ? (
                    <main className="space-y-8">
                        ...
                        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-lg font-semibold text-gray-700">Retention Rate</h3>
                                ...
                                <div className="mt-2 flex items-center text-sm">
                                    <span className={`mr-2 ${retentionRate >= 0.85 ? 'text-green-500' : 'text-red-500'}`}>
                                        {retentionRate >= 0.85 ? '▲' : '▼'} 
                                    </span>
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-lg font-semibold text-gray-700">YoY Change</h3>
                                    <TrendingUp size={24} className="text-blue-400" />
                                </div>
                                <p className={`text-3xl font-bold ${
                                    enhancedData?.yoyComparison?.accountsGrowth >= 0 ? 'text-red-600' : 'text-green-600'
                                }`}>
                                    {formatPercentage(Math.abs(enhancedData?.yoyComparison?.accountsGrowth || 0))}
                                </p>
                                <div className="mt-2 flex items-center text-sm">
                                    <span className={`mr-2 ${
                                        enhancedData?.yoyComparison?.accountsGrowth >= 0 ? 'text-red-500' : 'text-green-500'
                                    }`}>
                                        {enhancedData?.yoyComparison?.accountsGrowth >= 0 ? '▲' : '▼'} 
                                    </span>
                                    <span className="text-gray-500">
                                        {enhancedData?.yoyComparison?.accountsGrowth >= 0 ? 'Increase' : 'Decrease'} from last year
                                    </span>
                                </div>
                            </div>
                            {/* Total MKIB */}
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center justify-between">
                                <div>
                                    <h3 className="text-lg font-medium text-gray-500">Total MKIB</h3>
                        ...
                        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center justify-between">
                                <div>
                                    <h3 className="text-lg font-medium text-gray-500">Total MKFS</h3>
                                    <p className="text-3xl font-bold text-gray-900 mt-2">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center justify-between">
                                <div>
                                    <h3 className="text-lg font-medium text-gray-500">Total MKIC</h3>
                                    <p className="text-3xl font-bold text-gray-900 mt-2">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col">
                                ...
                                <p className="text-sm text-gray-500 mt-2">Potential annual impact</p>
                            </div>
                        </section>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-xl font-bold text-gray-800">Lost Accounts by Reason</h2>
                                ...
                                <div className="mt-4 text-sm text-gray-500">
                                    <p>Identifies the primary reasons for client churn with exact numbers</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-xl font-bold text-gray-800">Business Lost to Competitors</h2>
                                    <Info size={18} className="text-gray-400" />
                                </div>
                                ...
                                <div className="mt-4 text-sm text-gray-500">
                                    <p>Shows which competitors are winning our clients with exact values</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Additional Insights */}
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                            <h2 className="text-xl font-bold text-gray-800 mb-4">Retention Insights</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <h3 className="font-medium text-gray-700 mb-3">Top 5 Churn Reasons</h3>
                                        ...
                                        {sortedReasons.slice(0, 5).map((reason, index) => (
                                            <li key={index} className="flex items-center">
                                                ...
                                                <span className="ml-auto text-gray-600">
                                                    {reason.value} accounts ({Math.round((reason.value / totalLostAccounts) * 100)}%)
                                                </span>
                                            </li>
                                        ))}
                                    </ul>
                                        ...
                                        {sortedReasons.slice(0, 2).map((reason, index) => (
                                            <li key={index}>
                                                {reason.name === 'Competition' && (
                                                    <span>Enhance competitive pricing strategies for at-risk accounts</span>
                                                )}
                                                {reason.name === 'Financial Constraints' && (
                                                    <span>Develop flexible payment options for financially constrained clients</span>
                                                )}
                                                {reason.name === 'Bancassurance' && (
                                                    <span>Strengthen relationships with bank partners to retain bancassurance clients</span>
                                                )}
                                                {!['Competition', 'Financial Constraints', 'Bancassurance'].includes(reason.name) && (
                                                    <span>Implement targeted retention programs for {reason.name.toLowerCase()} cases</span>
                                                )}
                                            </li>
                                        ))}
                                        <li>Conduct exit interviews to better understand churn drivers</li>
                    ...
                    <main className="space-y-8">
                        ...
                        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-xl font-bold text-gray-800">Year on Year Comparison (2024 vs 2025)</h2>
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                ...
                                {enhancedData?.monthlyAnalysis?.length > 0 ? (
                                    <div className="h-96">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={sortDataByMonth(enhancedData?.monthlyAnalysis)}>
                                                <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                                                <XAxis 
                                                    dataKey="month" 
                                                    tick={{ fill: '#6b7280' }}
                        ...
                        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-xl font-bold text-gray-800">Lost Accounts by Division (2025)</h2>
                                        ...
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                ...
                                                <Tooltip 
                                                    formatter={(value, name) => {
                                                        if (name === 'accounts') {
                                                            const percentage = ((value / enhancedData.totalLostAccounts) * 100).toFixed(1);
                                                            return [`${value} accounts (${percentage}%)`, 'Accounts'];
                                                        }
                                                        return [value, name];
                                                    }}
                                                    contentStyle={{ 
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold text-gray-800">Year-over-Year Comparison</h2>
                                ...
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="text-center p-6 bg-red-50 rounded-lg border border-red-100">
                                            <p className="text-sm text-gray-600 mb-2">2025 Accounts</p>
                                            <p className="text-3xl font-bold text-red-600">
                                                {enhancedData.yoyComparison.currentYear.accounts}
                                            </p>
                                            <p className="text-sm text-gray-500 mt-2">
                                                {formatCurrency(enhancedData.yoyComparison.currentYear.revenue)}
                                            </p>
                                        </div>
                                        <div className="text-center p-6 bg-blue-50 rounded-lg border border-blue-100">
                                            <p className="text-sm text-gray-600 mb-2">2024 Accounts</p>
                                            <p className="text-3xl font-bold text-blue-600">
                                                {enhancedData.yoyComparison.lastYear.accounts}
                                            </p>
                                            <p className="text-sm text-gray-500 mt-2">
                                                {formatCurrency(enhancedData.yoyComparison.lastYear.revenue)}
                                            </p>
                                        </div>
                                        <div className={`text-center p-6 rounded-lg border ${
                                            ...
                                            <p className={`text-3xl font-bold ${
                                                ...
                                                {formatPercentage(Math.abs(enhancedData.yoyComparison.accountsGrowth))}
                                                {enhancedData.yoyComparison.accountsGrowth >= 0 ? ' ▲' : ' ▼'}
                                            </p>
                                            <p className={`text-sm mt-2 ${
                                                enhancedData.yoyComparison.accountsGrowth >= 0 ? 'text-red-500' : 'text-green-500'
                                            }`}>
                                                {enhancedData.yoyComparison.accountsGrowth >= 0 ? 'Increase' : 'Decrease'} from 2024
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                        <div className="text-center p-4 bg-gray-50 rounded-lg">
                                            <p className="text-sm text-gray-600 mb-2">Revenue YoY Change</p>
                                            <p className={`text-xl font-bold ${
                                                enhancedData.yoyComparison.revenueGrowth >= 0 ? 'text-red-600' : 'text-green-600'
                                            }`}>
                                                {formatPercentage(Math.abs(enhancedData.yoyComparison.revenueGrowth))}
                                                {enhancedData.yoyComparison.revenueGrowth >= 0 ? ' ▲' : ' ▼'}
                                            </p>
                                        </div>
                                        <div className="text-center p-4 bg-gray-50 rounded-lg">
                                            <p className="text-sm text-gray-600 mb-2">Average Account Value Change</p>
                                            <p className="text-xl font-bold text-gray-700">
                                                {formatPercentage(
                                                    (enhancedData.yoyComparison.currentYear.revenue / enhancedData.yoyComparison.currentYear.accounts) /
                                                    (enhancedData.yoyComparison.lastYear.revenue / enhancedData.yoyComparison.lastYear.accounts) - 1
                                                )}
                                            </p>
                                        </div>
                                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div className="p-6 border-b border-gray-200">
                            <div className="flex justify-between items-center">
                                <div>
                                    <h2 className="text-xl font-bold text-gray-800">Lost Accounts Details</h2>
                                    <p className="text-gray-600">Detailed view of all lost accounts</p>
                                </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {(() => {
                                        const filteredData = detailsYearFilter === 'all' 
                                            ? rawData 
                                        ...
                                        return currentItems.map((item, index) => (
                                            <tr key={index} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.competitor || 'Unknown'}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.reason || 'Unknown'}</td>
