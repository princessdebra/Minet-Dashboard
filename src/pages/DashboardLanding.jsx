...
import {
  ...
  Download, CheckCircle, XCircle, LogOut
} from 'lucide-react';
...
const DashboardLanding = ({ onNavigate, dashboardData, isLoading, error, onLogout, currentUser }) => { 
  ...
  const processDashboardData = (data) => {
    ...
    const lostAccountsCount = data.lostAccounts?.filter(acc => acc.amount && acc.client).length || 0;
    const lostAccountsRevenue = data.totalLostRevenue || 0;
    
    // Calculate win rate
    const totalOpportunities = wonAccountsCount + lostAccountsCount;
    const winRate = totalOpportunities > 0 ? wonAccountsCount / totalOpportunities : 0;
    
    // Get market share data
    const marketShareValue = data.marketShare?.[0]?.minet_market_share_q4_2023 || 0;
    ...
    if (dashboardData) {
      ...
      const animate = (currentTime) => {
        ...
        setAnimatedValues({
          ...
          wonAccountsRevenue: Math.floor(progress * (processedData.wonAccountsRevenue || 0)),
          lostAccounts: Math.floor(progress * (processedData.lostAccounts || 0)),
          lostAccountsRevenue: Math.floor(progress * (processedData.lostAccountsRevenue || 0)),
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center min-h-screen text-xl text-gray-700">Loading...</div>;
  }
  ...
  const processedData = processDashboardData(dashboardData);
  if (!processedData) {
    return <div className="flex justify-center items-center min-h-screen text-xl text-gray-500">
      Dashboard data is not in the expected format. Please check the backend API.
    </div>;
  }
  ...
  const allMonths = new Set([...Object.keys(wonByMonth), ...Object.keys(lostByMonth)]);
...
const prepareRevenueByDivision = () => {
  ...
  const revenueByDivision = dashboardData.wonAccounts.reduce((acc, account) => {
  ...
  const salesPipelineData = dashboardData.wonAccounts
      ...
      <div> 
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-4xl font-extrabold text-gray-900 leading-tight">
            Minet CIA <span className="text-red-600">Analytics Dashboard</span>
          </h1>
        </div>
    ...
    <div className="flex items-center space-x-4">
      {currentUser?.role === 'admin' && (
        <button 
          ...
          className="flex items-center px-4 py-2 bg-white text-gray-700 rounded-lg shadow-sm hover:bg-gray-50 transition-colors border border-gray-300"
        >
          <Shield size={20} className="mr-2" /> Manage Users
        </button>
      )}
      <button className="flex items-center px-4 py-2 bg-white text-gray-700 rounded-lg shadow-sm hover:bg-gray-50 transition-colors border border-gray-300">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-gray-900">
              Hello {currentUser.full_name || currentUser.name || 'User'}!
            </p>
          </div>
        </div>
        ...
        <main>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-200">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-gray-800">Revenue by Division</h2>
              </div>
              <ResponsiveContainer width="100%" height={300}>
                {revenueByDivisionData.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-full text-gray-500">No revenue by division data available.</div>
                )}
              </ResponsiveContainer>
              <div className="mt-6 text-base text-gray-600 text-center">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-200">
              <h2 className="text-xl font-bold text-gray-800 mb-4">Recent Won Accounts</h2>
              <div className="overflow-x-auto">
                {salesPipelineData.length > 0 ? (
                  ...
                  <p className="text-gray-500">No recent won accounts data available.</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-200">
              <h2 className="text-xl font-bold text-gray-800 mb-4">Market Share by Category</h2>
              <ResponsiveContainer width="100%" height={300}>
                {dashboardData.marketShare?.length > 0 ? (
                  <PieChart>
                    <Pie
                      data={dashboardData.marketShare.filter(item => item.minet_market_share_q4_2023 !== null).slice(0, 5)}
                      cx="50%"
                      cy="50%"
                      labelLine={false}
                      outerRadius={80}
                  ...
                  <div className="flex items-center justify-center h-full text-gray-500">No market share data available.</div>
                )}
              </ResponsiveContainer>
              <div className="mt-6 text-base text-gray-600 text-center">
